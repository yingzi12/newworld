...
public class DiffHandleUtils {
    ...
    public static List<String> diffString(List<String> original, List<String> revised, String originalFileName, String revisedFileName) {
        ...
        System.out.println("两文件的不同点 patch:" + JSONObject.toJSONString(patch));
        //生成统一的差异格式
        List<String> unifiedDiff = UnifiedDiffUtils.generateUnifiedDiff(originalFileName, revisedFileName, original, patch, 0);
        System.out.println("生成统一的差异格式 unifiedDiff:" + JSONObject.toJSONString(unifiedDiff));
    ...
    public static void main(String[] args) {
        //对比 F:\n1.txt和 F:\n2.txt 两个文件，获得不同点
        List<String> diffString = DiffHandleUtils.diffString("/Users/luhuang/Documents/test1", "/Users/luhuang/Documents/test2");
        //在F盘生成一个diff.html文件，打开便可看到两个文件的对比
        DiffHandleUtils.generateDiffHtml(diffString, "/Users/luhuang/Documents/test3.html");

        DiffRowGenerator generator = DiffRowGenerator.create()
                .showInlineDiffs(true)
                .inlineDiffByWord(true)
//                .oldTag(f -> f ? "<span class=\"editOldInline\">" : "</span>")
//                .newTag(f -> "**")
                .build();
        List<DiffRow> rows = generator.generateDiffRows(
                Arrays.asList("", "And here is the finish."),
